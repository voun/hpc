...
char *COLORS[] = {"9 0 0 ", "0 9 0 ", "9 5 3 ", "9 9 0 ", "0 9 9 ", "0 5 5 ", "0 5 0 ", "5 0 5 ", "0 0 5 ", "10 0 10 ", "0 0 10 "};
//pre-format strings for speed
char *ITERATIONS[] = {"00 00 00 ", "01 01 01 ", "02 02 02 ", "03 03 03 ", "04 04 04 ", "05 05 05 ", "06 06 06 ", "07 07 07 ", "08 08 08 ", "09 09 09 ",
...
int main(int argc, char *argv[]){
    ...
    if(DEGREE == 1){
        ...
        char *attr_string = malloc(sizeof(char)*6*LINES*LINES);
        char *conv_string = malloc(sizeof(char)*12*LINES*LINES);

        for(unsigned int i = 0; i < LINES; i++){
            for(unsigned int j = 0, k1=0, k2=0; j < LINES; j++, k1+=9, k2+=6){

                memcpy(conv_string + k1, ITERATIONS[1], 9);
                memcpy(attr_string + k2, COLORS[1], 6); 
            }                          
        }

        fwrite(attr_string, sizeof(char), 6*LINES*LINES, attr_ptr);    
        fwrite(conv_string, sizeof(char), 12*LINES*LINES, conv_ptr);
        fclose(attr_ptr);
        fclose(conv_ptr);
        return 0;
    ...
    for(unsigned int i = 0; i < LINES; i++){
        ...
        attractors[i][LINES-1] = 11; //11 means this line not finished
        attractors_strings[i] = malloc(sizeof(char)*9*LINES);
        convergences[i] = malloc(sizeof(unsigned int)*LINES);
        convergences_strings[i] = malloc(sizeof(char)*15*LINES);
    }
...
void *write_fun(void *arg){
    ...
    for(unsigned int i = 0; i < LINES;){
        ...
        if(attractors[i][LINES-1] == 11){ //wait until finished
            nanosleep(&sleepvalue, NULL);
            continue;
        }

        for(; i < LINES && attractors[i][LINES-1] != 11; i++){
       
            fwrite(attractors_strings[i], sizeof(char), 6*LINES, attr_ptr);    
            fwrite("\n", sizeof(char), 1, attr_ptr);  
               
            fwrite(convergences_strings[i], sizeof(char), 12*LINES, conv_ptr);
